...
## The following code takes in gene trees from two sources: (1) N-mt genes (n=167) and (2) control genes
## (n=142) and compares them (by calculating Robinson-Foulds [RF] distances and average bootstrap support values) to three
## topologies: (1) the species tree, (2) the mitochondrial tree, and (3) the ILS tree. It also calculates
## weighted RF distances (i.e., taking into account branch lengths).

## Some of the following code (and functions, in `RFdists.R`), particularly code to order loci according to RFs, comes
## directly from the genesortR package available here: https://github.com/mongiardino/genesortR.

##    FILES REQUIRED:
##          Gene trees from N-mt genes ("nmt_3taxa_genetrees.txt") and control genes ("cont_3taxa_genetrees.txt")
##          Topologies for species, mitochondrial, and ILS trees ("panther_speciestree.txt", "panther_mitotree.txt", and "panther_ILStree.txt")
...
dat <- read_files(data_path = "RF_dists/", 
