...
## The following code is largely adapted from Misha Matz and runs a constrained
## ordination-based GWAS analysis. Misha's code is available here:
## https://github.com/z0on/Multivariate_GWAS
## It calculates SNP scores, empirical p-values, betas, and r-squares for RDA-based
## GWAS. Per-SNP values are computed against the first constrained ordination axis.

##    FILES REQUIRED:
##      `gt=[filename]` genotypes   Table of minor allele counts
##      `covars.e=[filename]`       Table of non-genetic covariates
##      `covars.g=[filename]`       Table of genetic covariates
##      `traits=[filename]`         Table of trait(s)
##      `gdist=[filename]`          Matrix of genetic dists between samples (IBS matrix)
##      `gdist.samples=[filename]`  List of sample names
##      `badsites=[filename]`       List of sites to exclude
##      `hold.out=[filename]`       List of samples to hold out from analysis for testing
##      `plots=TRUE/FALSE`          Whether to build plots
##      `nsites=5500000`            Number of sites to compute FDR
##      `prune.dist=50000`          Pruning distance
...
##    STRUCTURE OF CODE:
##      (1-4) Pre-processing data files
##      (5-6) Running the GWAS
##      (7-9) Compute results and save

# Load GWAS-related functions
...
# Reading in bad sites and removing them
...
# 5. Regressing enviro covariates out of traits ---------------------------
...
# 6. Computing RDA and SNP scores against CAP1 ----------------------------
...
# 7. Build q-q plot to check for signal -----------------------------------
