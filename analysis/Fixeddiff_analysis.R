...
mitotypes <- read_tsv(here("Fixed_diffs", "cz_mitotypes.txt"), col_names = FALSE) %>% 
  rename(INDV = X1, mitotype = X2)

### Run above function for NMTs and control loci
nmts <- process_seq(vcf_file = here("Fixed_diffs", "cznmtsnps.vcf"),
                    fasta_file = here("Fixed_diffs", "cznmtsnps.min4.fasta"),
                    mitotypes = mitotypes) # 30 inds of 38,553 vars (38,551 SNPs)
cont <- process_seq(vcf_file = here("Fixed_diffs", "czcontsnps.vcf"),
                    fasta_file = here("Fixed_diffs", "czcontsnps.min4.fasta"),
...
load(paste0(here("Fixed_diffs"), "/pure_allele_dict_", dataset_name, ".rda"))
...
results <- freq_data(dataset, 
                     ...
                     threshold, 
                     save_file = FALSE, 
                     output_path = paste0(here("Fixed_diffs"), "/"))
