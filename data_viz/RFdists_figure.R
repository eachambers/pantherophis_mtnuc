...
## The following code takes in results from RF distances analysis and plots stacked barplots based on proportions of
## N-mt and control gene trees that match to the species, mitochondrial, or ILS trees (Fig. X).

##    FILES REQUIRED:
##          `cont_results` and `nmt_results` from RFdists_analysis.R script

##    STRUCTURE OF CODE:
##              (1) Process data a bit
##              (2) Build plot
...
plot_dat %>% 
  ...
  scale_fill_manual(values = c("mito" = "#6b8577", "ILS" = "#2b5ba4", "spp" = "#95627a")) +
